<?php
    ...
    if(isset($_POST["crear"])){
        if(preg_match('/^[a-zñÑ A-Z]+$/',$_POST["nombre"])== 1){
            $nombre = $_POST["nombre"];
        }else{echo '<script>confirm("Por favor ingrese un nombre valido")</script>';}

        if(preg_match('/^[0-9A-Za-z]+$/',$_POST["usuario"])== 1){        
            $usuario = $_POST["usuario"];
        }else{echo '<script>confirm("Por favor ingrese un usuario valido")</script>';}

        $direccion= ($_POST["direccion"]);

        if(preg_match('/^[0-9]+$/',$_POST["telefono"])== 1){
            $telefono = $_POST["telefono"];
        }else{echo '<script>confirm("Por favor ingrese un número de telefono valido")</script>';}

        if(preg_match("/(^([A-Za-z.]|@|[0-9])+$)/",$_POST["email"])== 1){
            $email = $_POST["email"];
        }else{echo '<script>confirm("Por favor ingrese un email valido")</script>';}
